...
const CardsContainer: React.FC = () => {
  ...
  const [poapData, setPoapData] = useState<Record<string, Card>>({}); // Agregamos el estado para almacenar los datos de POAP
  ...
  const openCardDrawer = (card: Card) => {
  ...
  const renderCardContent = (card: Card) => (
    <>
      <Flex align="center" justify="center">
        <Image src={card.imageUrl} alt={card.title} objectFit="cover" rounded="full"   borderRadius='full'
  boxSize='220px' />
      </Flex>
        ...
        {Object.keys(jsonData).map((id) => {
          ...
          return (
            <div key={id} className="w-full sm:w-1/4 md:w-1/2 lg:w-1/2 xl:w-1/2 p-4" onClick={() => openCardDrawer(card)}>
              <div className=" bg-gradient-to-br from-indigo-600 to-green-600 py-6 flex flex-col justify-center sm:py-12 p-10 rounded-300 shadow-md">
